...
canvas.addEventListener("mousedown", (event) => {
  ...
  for (let i = 0; i < 5; i++) { // Générer 5 gouttes au début
    ...
    console.log(`knifeX: ${knifeX}, knifeY: ${knifeY}`);
...
function update(dt) {
  ...
  if (isFilling) {
    for (let i = 0; i < 5; i++) { // Ajouter 5 gouttes supplémentaires à chaque mise à jour pendant que le clic est maintenu
      ...
      drops.push({ x: dropX, y: dropY, height: 0, speed: dropSpeed, size : dropSize}); // Ajoutez speed ici
    }
  }

  // Mettre à jour la hauteur des gouttes d'eau
  drops.forEach((drop) => {
    // Augmenter la hauteur de la goutte pour simuler un écoulement
    if (drop.height < canvas.height) { // Limiter la hauteur maximale
      drop.height += drop.speed; // Utilisez la vitesse de la goutte pour ajuster sa hauteur
    }
  });
